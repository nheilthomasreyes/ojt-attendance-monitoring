...
/** * CONFIGURATION: 
 * Update this to a local IP address of a server inside your office.
 * Example: 'http://192.168.1.50/ping' or just the gateway 'http://192.168.1.1'
 */
const OFFICE_BEACON_URL = 'http://192.168.1.50/ping'; 
...
export function NetworkDetector({ officeSSID, onNetworkDetected }) {
  ...
  // 2. Define detectNetwork (iOS & Android Compatible)
  const detectNetwork = useCallback(async () => {
    setIsScanning(true);

    // Get Connection Type (Android/Chrome only usually)
    const nav = navigator;
    ...
    // Beacon Check: This works on iOS because it checks if the server is REACHABLE
    // instead of trying to read the device's private IP.
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 3500); // 3.5s timeout

      await fetch(OFFICE_BEACON_URL, { 
        mode: 'no-cors', 
      ...
      const networkName = 'Restricted / External';
      setDetectedNetwork(networkName);
      updateStatus(false, networkName);
    } finally {
      setIsScanning(false);
  ...
  useEffect(() => {
    detectNetwork();
    const interval = setInterval(detectNetwork, 10000); // Re-scan every 10s
    return () => clearInterval(interval);
  }, [detectNetwork]);
